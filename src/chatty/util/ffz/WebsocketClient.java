...
public class WebsocketClient {
   ...
   public synchronized String getStatus() {
      ...
      if (s != null && s.isOpen()) {
         return String.format("Connected for %s\n"
               ...
               lastMeasuredLatency,
               timeLatencyMeasured == 0 ? "not yet measured" : "measured " + DateTime.ago(timeLatencyMeasured) + " " +
                     "ago");
      }
      return "Connecting..";
   }
   ...
   private synchronized void send(String text) {
      if (s != null && s.isOpen()) {
         ...
         //REMOVE
         if (Chatty.DEBUG)
            System.out.println("WEBSOCKET SEND: " + text);
   ...
   public synchronized void onMessage(String message, Session session) {
      //REMOVE
      if (Chatty.DEBUG)
         System.out.println("WEBSOCKET DATA: " + message);
